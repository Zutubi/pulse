...
public NotifyConditionTreeParser() {
	...
	public final NotifyValue  value(AST _t) throws RecognitionException {
		NotifyValue r;
		...
		try {      // for error handling
			...
			case 26:
			{
				...
				strsymbol(_t);
				_t = _retTree;
				r = factory.createStringValue(k.getText());
				break;
			}
			case WORD:
			{
				...
				match(_t,WORD);
				_t = _t.getNextSibling();
				r = new LiteralNotifyStringValue(l.getText());
				break;
			}
			default:
			{
	...
	public final void strsymbol(AST _t) throws RecognitionException {
		
		AST strsymbol_AST_in = (_t == ASTNULL) ? null : (AST)_t;
