...
public class CustomHttpClientHandler extends HttpClientHandler
{
    ...
    {
        return (Boolean) invoke(METHOD_USE_PROXY_AUTHENTICATION);
    }

    private boolean callUseAuthentication(URL dest)
    {
        return (Boolean) invoke(METHOD_USE_AUTHENTICATION, dest);
    }

    private HttpClient callGetClient(URL dest)
    {
        return (HttpClient) invoke(METHOD_GET_CLIENT, dest);
    }
    ...
    private Object invoke(String methodName, Object... args)
    {
        try
        {
            ...
            Method method = HttpClientHandler.class.getDeclaredMethod(methodName, types);
            method.setAccessible(true);
            return method.invoke(this, args);
        }
        catch (Exception e)
        {
            throw new RuntimeException("Incompatible version of ivy.");
        }
    }
