...
public class PostProcessCommand implements Command
{
    ...
    private void postProcess(String processorName, String inputFile, String pulseFile, String resourcesFile, PrintStream out)
    {
        ...
        Scope scope = pf.getGlobalScope();
        Reference reference = scope.getReference(processorName);
        ...
        CommandContext context = new CommandContext(null, null, new TestSuiteResult());

        indenter.println("Running processor...");
        processor.process(artifact, result, context);
        indenter.println("Post-processing complete.");

        printResults(indenter, artifact, context.getTestResults());
    }
    ...
    private void printResults(Indenter indenter, StoredFileArtifact artifact, TestSuiteResult testResults)
    {
    ...
    {
        ConfigurableResourceRepository resourceRepository;
    ...
    private PulseFile loadPulseFile(String fileName, ResourceRepository resourceRepository)
    {
        ...
        try
        {
            loader.load(new FileInputStream(f), result, new Scope(), resourceRepository, null);
            return result;
        }
        catch (Exception e)
        {
