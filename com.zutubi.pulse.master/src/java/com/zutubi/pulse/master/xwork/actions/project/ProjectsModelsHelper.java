    ...
    private ProjectsModel createModel(String name, boolean labelled, Collection<Project> projects, TemplateHierarchy hierarchy, User loggedInUser, ProjectsSummaryConfiguration configuration, Set<String> collapsed, Map<Project, List<BuildResult>> buildCache, Urls urls)
    {
        ...
        else
        {
            for (Project p : projects)
            {
                ...
                ConcreteProjectModel child = new ConcreteProjectModel(model, p, builds, loggedInUser, configuration, urls, prompt, getAvailableActions(p), health);
                ...
                if (includedInGroup.contains(node.getId()))
                {
                    ...
                    if (node.isConcrete())
                    {
                        ...
                        if (project != null)
                        {
                            ...
                            model = new ConcreteProjectModel(group, project, builds, loggedInUser, configuration, urls, prompt, getAvailableActions(project), health);
                        }
                    }
                    else
                    {
