...
public class RefactorCleanupWhatFieldUpgradeTask extends AbstractRecordPropertiesUpgradeTask
{
    ...
    // C: change what to list, BUILD_ARTIFACTS -> BUILD_ARTIFACTS + WORKING_DIRECTORIES_ONLY etc.
    ...
    class CleanupRecordUpgrader implements RecordUpgrader
    {
        public void upgrade(String path, MutableRecord record)
        {
            ...
            {
                record.put(PROPERTY_WHAT, new String[]{CLEANUPWHAT_WORKING_DIRECTORIES_ONLY});
            }
            else if (what.equals(CLEANUPWHAT_BUILD_ARTIFACTS))
            {
                record.put(PROPERTY_WHAT, new String[]{CLEANUPWHAT_WORKING_DIRECTORIES_ONLY, CLEANUPWHAT_BUILD_ARTIFACTS});
            }
